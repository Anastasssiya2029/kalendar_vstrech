...
class ApiService {
  ...
  async createUser(userData: { email: string; name: string; schoolId: string; password: string }) {
    return this.request<{ user: any }>(this.schoolPath(userData.schoolId, "users"), {
      method: "POST",
      body: JSON.stringify({ email: userData.email, name: userData.name, password: userData.password }),
    });
  }

  async updateUser(schoolId: string, userId: string, data: { name?: string; email?: string; password?: string }) {
